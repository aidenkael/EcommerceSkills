...
核算模式只在当前对话有效。模式状态仅存在于当前聊天中，不保存到任何文件。

新对话第一次核算：只输出模式菜单并停止，不提前识图或计算。禁止"需要先确认模式"后继续计算。

同一对话已选模式后：后续任何商品直接沿用，不再询��。

`config/local_user_preferences.json` 只保存四项利润参数。不保存、读取或推断 `active_mode`。

### 缺失参数自动估算

缺少重量/尺寸/数量不得弹窗或停下询问。

缺重量：AI根据材质/结构/体积估算净重，正常档用最可能值，保守档吸收不确定性。标记 `confidence=low`。
缺尺寸：根据物理结构估算运输包装，先判断折叠/压缩/刚性轴。
缺数量：默认 `quantity=1, quantity_source=assumed`。
...
日常核算先选一个最匹配结构类型，不逐类展开15种规则。常规核算不读案例库。

**pending 案例禁止：** 反推费用推算包装体积、反推包装尺寸、证明本次包装合理、覆盖当前图片和SKU证据。

### 固定输出检查

最终输出前必须确认：
- 严格使用固定四行表格（义乌正常/义乌保守/深圳正常/深圳保守）
- 没有额外"推荐"列
- 没有临界点分析
- 没有历史案例长段落
- 没有工作日志或内部执行过程
...
Agent 不得主要按商品名称直接套规则。每件商品必须先拆分为物理结构。

### 结构分解清单

每件商品先判断：
1. 刚性主体 — 不可压缩/不可折叠的核心部分
2. 可折附属结构 — 把手/帽檐/可弯曲部件
3. 可拆卸附件 — 肩带/链条/配饰
4. 可压缩空腔 — 空心包体/毛绒填充
5. 柔性填充部分 — 海绵/棉花/软胶
6. 不可压缩突出件 — 硬手柄/硬底座
...
见 `knowledge/physical_packaging_rules.json`，共 15 种类型。Agent 应根据图片选择最接近的类型。

### 展示尺寸与运输尺寸分离

所有品类都必须执行：**展示尺寸 ≠ 运输包装尺寸**。

必须判断：哪些部件只在展示时展开、哪些运输时可折叠、哪些可拆卸、哪些空腔可压缩、哪些多件可嵌套。

各品类关键规则：
- **包类**：把手可折叠不计入刚性高度；肩带可拆卸或盘绕不计入最长外廓；空心软包可适度压缩厚度；硬底/硬框决定最低厚度
- **帽子**：帽冠可轻压减少高度；硬帽檐和定型结构不得折叠
- **毛绒/纺织品**：判断压缩率和回弹风险；展开尺寸不得直接作为包装尺寸
- **雨伞/数据线/项链/腰带**：展开长度不得作为包装最长边；按收拢/盘绕/折叠状态计算
- **关节支架/灯具/可折工具**：判断关节是否可折叠；底座和硬杆决定不可压缩外廓
- **多件套**：不得简单相加每件外廓；先判断嵌套/套叠/错位

### 包装方式选择

没有明确包装证据时，按商品物理需求选择，不按"看起来高档"选择。

默认候选层级：OPP袋/防尘袋 → 快递袋 → 薄气泡袋 → 局部五金保护 → 薄纸盒 → 硬纸盒

禁止无证据默认：礼盒、品牌零售包装、四周3～5cm缓冲、硬纸箱保型。

### 异常复核机制

触发条件（生成 AI JSON 前轻量检查）：
- 单件商品被识别成多件套
- 展示外廓全部直接进入包装尺寸
- 可折部件仍被计入最长轴
- 空心柔性商品被判断为完全不可压缩
...
触发后：重新检查 SKU → 重新分解物理结构 → 重新生成包装方案一次。最多重算一次。仍无法确认时降低置信度并标记复核。
